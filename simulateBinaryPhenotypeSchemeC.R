...
for (replicateIndx in 1:1000) {
  ...
  controlSNPIndx <- sort(sample(controlIndividualIndx[y[controlIndividualIndx]==0], size=length(caseSNPIndx), replace=FALSE))
  controlFamilyfromSNPIndx <- unique(sapply(controlSNPIndx, function(x){ifelse(x%%10==0, x%/%10, x%/%10+1)}))
  for (famIndx in 1:length(controlFamilyfromSNPIndx)) {
    sumOfK <- sumOfK + 1
  }
  KTrace[replicateIndx, ] <- c(sumOfK, length(c(diseaseFamilyIndx, controlFamilyfromSNPIndx)))
