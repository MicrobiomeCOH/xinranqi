...
for (replicateIndx in 1:1000){
  ...
  logitNull <- lm(Y~X1, data=phenoSTAAR)
  ...
  for (genoIndx in 1:ncol(G)) {
    ...
    # test for SNPs
    logitFull <- lm(Y~X1+geno, data=phenoSTAARGeno)
    # Wald test 
    waldTemp <- ifelse(length(coef(logitFull))==3, summary(logitFull)$coefficients[3, 4], 
  ...
  WaldY.res <- phenoSTAAR$Y - logitNull$fitted.values
  ...
  colnames(singleResult.detail)<-c('replicate','h2','N.causal','fdr',
                                   ...
                                   'Ghostknockoff_Wald_power', 'Ghostknockoff_LRT_power', 
                                   ...
                                   'Ghostknockoff_Wald_fdp', 'Ghostknockoff_LRT_fdp')
