...
#setwd('/scratch/users/xinranqi/Project1/CCFPlan10')
...
for (replicateIndx in 1:1000) {
  ...
  # randomly select cases & controls without looking at family
  ...
  sumOfK <- 0
  ccIndivIndx <- sort(c(caseIndx, controlIndx))
  familyfromSNPIndx <- unique(sapply(ccIndivIndx, function(x){ifelse(x%%10==0, x%/%10, x%/%10+1)}))
  for (famIndx in 1:length(familyfromSNPIndx)) {
    yPerPedigree <- ccIndivIndx %in% ((familyfromSNPIndx[famIndx]-1)*10+1):(familyfromSNPIndx[famIndx]*10)
    yPerPedigree <- y[ccIndivIndx[yPerPedigree]]
    sumOfK <- if(length(yPerPedigree)==1) {
      sumOfK
    } else {
      sumOfK + sum(sapply(1:(length(yPerPedigree)-1), function(x){sum(yPerPedigree[x] == yPerPedigree[(x+1):length(yPerPedigree)])}))/choose(length(yPerPedigree), 2)
    }
  }
  KTrace[replicateIndx, ] <- c(sumOfK, length(familyfromSNPIndx))
  ...
  kinshipMatrix <- kinshipMatrix[c(caseIndx, controlIndx), c(caseIndx, controlIndx)]
