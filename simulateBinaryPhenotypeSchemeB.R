...
#setwd('/scratch/users/xinranqi/Project1/CCFPlan8')
...
KTrace <- matrix(NA, ncol=2, nrow=1000)
...
for (replicateIndx in 1:1000) {
  ...
  # detect sum of affected family members
  sumDiseasePerFamily <- sapply(1:numFamilies, function(x){sum(y[((x-1)*10+1):(x*10)])})
  diseaseFamilyIndx <- sort(sample(which(sumDiseasePerFamily>=1), 500, replace=FALSE), decreasing=FALSE)   
  # balance disease & control with all control family
  controlFamilyIndx <- if (sum(sumDiseasePerFamily==0) < 500) {
    sort(sample(setdiff(which(sumDiseasePerFamily %in% c(0,1)), diseaseFamilyIndx), 500, replace=FALSE), decreasing=FALSE)   
  } else {
    sort(sample(which(sumDiseasePerFamily == 0), 500, replace=FALSE), decreasing=FALSE)   
  }
  
  # estimate relatedness measurement
  sumOfK <- 0
  for (pedigreeIndx in c(diseaseFamilyIndx, controlFamilyIndx)) {
    yPerPedigree <- y[((pedigreeIndx-1)*10+1):(pedigreeIndx*10)]
    sumOfK <- sumOfK + sum(sapply(1:(length(yPerPedigree)-1), function(x){sum(yPerPedigree[x] == yPerPedigree[(x+1):length(yPerPedigree)])}))/choose(length(yPerPedigree), 2)
  }
  KTrace[replicateIndx, ] <- c(sumOfK, length(c(diseaseFamilyIndx, controlFamilyIndx)))
  write.table(KTrace,paste0('/oak/stanford/groups/zihuai/XinranQi/Project1/',location,'/singleResult_knockoff_KTrace_detail.txt'),row.names=F,col.names=T,quote=F,sep='\t')
  
  ccIndivIndx <- sort(sapply(c(diseaseFamilyIndx, controlFamilyIndx), function(x){((x-1)*10+1):(x*10)}))
  G <- G[ccIndivIndx, ]
  X1 <- X1[ccIndivIndx]
  y <- y[ccIndivIndx]
  ...
  G_k <- KnockoffScreen::create.MK(G,pos,M=5,maxN.neighbor=Inf,maxBP.neighbor=0.1*10^6)
  ...
  # fit null model
  fit.prelim.Ghost <- GhostKnockoff.prelim(cor.G,M=5,method='sdp')
